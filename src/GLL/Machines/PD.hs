...
gll flags m debug (start, prods) input = 
    (runGLL (pLhs (start, 0, (U0,0))) flags context, selects, follows)
 where 
    context = Mutable False emptySPPF [] IM.empty IM.empty IM.empty IM.empty

    recurse :: Parseable t => Rcal t -> Ucal t -> GSS t -> Pcal t -> SPPF t -> (Mutable t)
    recurse [] u g p d = Mutable success d [] u g p mismatches
      where success = undefined 
            mismatches = undefined
    recurse (((slot,i,gn),snode):rest) u g p d = 
        consider (process (slot,i,gn)) rest u g p d

        where process (slot, i, (g, l)) = case slot of
                Slot x alpha []             -> [] -- pop
                Slot x alpha (Nt y:beta)    -> [] -- nt
                Slot x alpha (Term t:beta)  -> []
        
              consider [] r u g p d = recurse r u g p d 
              consider (desc@((slot,i,(gslot,l)),snode):ds) r u g p d 
                | new = consider ds (desc:r) u g p d
                where new     = maybe True inner $ IM.lookup i u
                      inner m = maybe True (not . ((slot,gslot) `S.member`)) $ IM.lookup l m
